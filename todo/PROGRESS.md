...
- [x] P02-4：K线来源统一（决策/看板/OPN 一致性；支持 Binance-first + fallback）
...
- 日期：2026-01-03
- 备注：已新增 `MARKET_KLINE_SOURCE=auto|binance|coinank`，策略输入与K线看板默认走同一来源；K线缓存按“下一根 bar 出现”刷新以降低重复拉取。
...
- [x] P03-5：MinConfidence 代码硬闸门 + prompt_variant 持久化并生效
...
- 日期：2026-01-03
- 备注：`risk_control.min_confidence` 变为后端硬闸门（低置信开仓直接拒绝）；`prompt_variant` 现在写入 strategy config 并在 AutoTrader 调 AI 时生效；同时下调新建策略的默认杠杆/仓位上限，减少过度交易风险。
